...
export const useStreamingChat = (
  ...
  const startStreaming = useCallback(async (userInput: string) => {
    ...
    try {
      ...
      const stream = streamingChatService.streamConversation(
        sessionId,
        userInput,
        userId
      );
      ...
      for await (const event of stream) {
        if (event.type === 'message_chunk') {
          ...
          setCurrentStreamingMessage(prev => {
            if (!prev) return null;
            const errorMessage = completeMessage(updateMessageContent(prev, `Error: ${event.content}`));
            setMessages(prevMessages => {
              const validMessages = filterValidMessages(prevMessages);
              return validMessages.map(msg => 
                msg.id === prev.id ? errorMessage : msg
              );
            });
            return null;
          });
      ...
      setCurrentStreamingMessage(prev => {
        if (!prev) return null;
        const errorMessage = completeMessage(updateMessageContent(prev, 'Sorry, I encountered an error. Please try again.'));
        setMessages(prevMessages => {
          const validMessages = filterValidMessages(prevMessages);
          return validMessages.map(msg => 
            msg.id === prev.id ? errorMessage : msg
  ...
  }, [isStreaming, sessionId, userId, addMessage, onMessageComplete, onValuationComplete, onReportUpdate, onProgressUpdate]);
