...
export const useSuggestionHandlers = ({
  ...
  const handleKBOSuggestionSelect = useCallback(async (selection: string) => {
    ...
    try {
      ...
      await streamingManager.startStreaming(
        ...
        (error: Error) => {
          chatLogger.error('Streaming error during KBO selection', { error: error.message, sessionId: effectiveSessionId });
  ...
  }, [sessionId, pythonSessionId, userId, streamingManager, eventHandler, setIsStreaming, setIsTyping, addMessage, updateStreamingMessage, onHtmlPreviewUpdate]);
