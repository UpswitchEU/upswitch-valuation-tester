...
function VersionTimelineItem({
  ...
  const changeSummaries = hasChanges
    ? formatChangesSummary(version.changesSummary, version.formData.country_code || 'BE')
    : []
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-1">
            <span
              className={`
                ...
                ${isActive ? 'bg-primary-500 text-white' : 'bg-gray-200 text-gray-700'}
              `}
            >
            ...
            {version.isPinned && <Pin className="w-4 h-4 text-amber-500" />}
          ...
          {!compact && hasChanges && changeSummaries.length > 0 && (
            <div className="mt-2 space-y-1">
              ...
              {changeSummaries.length > 3 && (
                <p className="text-sm text-gray-500">+{changeSummaries.length - 3} more changes</p>
              )}
            </div>
          )}
          ...
          {!compact && version.notes && (
            <p className="mt-2 text-sm text-gray-600 line-clamp-2 italic">"{version.notes}"</p>
          )}
        </div>
