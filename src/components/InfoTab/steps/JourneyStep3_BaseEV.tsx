...
export const JourneyStep3_BaseEV: React.FC<JourneyStep3Props> = ({ result }) => {
  ...
  // CRITICAL FIX: Don't return null - show step with error message instead
  // This ensures Step 3 always appears in the frontend, even if data is missing
  if (!multiples || !currentData || !hasValidData) {
    stepLogger.warn('JourneyStep3_BaseEV missing required data, showing error state', {
      step: 3,
    ...
    // Return error state component instead of null
    return (
      <div className="bg-white rounded-lg border-2 border-gray-300 shadow-sm overflow-hidden">
        <div className="bg-gray-50 p-4">
          <div className="flex items-center gap-3">
            <div className="bg-gray-500 text-white w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0">3</div>
            <div>
              <h3 className="text-lg font-bold text-gray-900">Base Enterprise Value</h3>
              <p className="text-sm text-gray-600 mt-0.5">Data unavailable</p>
            </div>
          </div>
        </div>
        <div className="p-6">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-yellow-800">
                  {!currentData 
                    ? "Current year financial data is missing. Please ensure revenue and EBITDA data are provided."
                    : !hasValidData
                    ? "Revenue and EBITDA data are required to calculate enterprise value. Please enter valid financial data."
                    : !multiples
                    ? "Industry multiples are not available. Please ensure benchmarking data is provided."
                    : "Insufficient data to calculate base enterprise value."}
                </p>
              </div>
            </div>
          </div>
