...
export const generateReportName = (reportId: string): string => {
  const trendWords = [
    'Wave', 'Pulse', 'Flow', 'Surge', 'Spike', 'Shift', 'Boom', 'Peak', 'Rise', 'Edge', 'Vibe', 'Buzz', 'Momentum', 'Force'
  ];

  const descriptors = [
    'Business', 'Company', 'Enterprise', 'Corporate', 'Valuation', 'Analysis', 'Assessment', 'Review', 'Study', 'Report'
  ];

  const formats = [
    'Valuation', 'Analysis', 'Report', 'Assessment', 'Study', 'Brief', 'Overview', 'Summary', 'Intelligence', 'Tracker'
  ];
