...
export const BusinessProfileSection: React.FC<BusinessProfileSectionProps> = React.memo(
  ({ showPreConversationSummary, onTogglePreConversationSummary }) => {
    ...
    const startIntelligentConversation = useCallback(
      async (profileData: BusinessProfileData) => {
        try {
          chatLogger.info('Starting intelligent conversation with pre-filled data')

          // Transform business data to conversation request
          const conversationRequest = businessDataService.transformToConversationStartRequest(
            profileData,
            {
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-primary-200 mb-3">
                    We found your business profile! We'll skip the questions we already know and
                    ...
                    return (
                      <div className="space-y-3">
                        ...
                        <div className="text-sm text-primary-200">
                          <span className="text-primary-300">Estimated time:</span>{' '}
                          ...
                          <div className="text-sm">
                            ...
                            <div className="flex flex-wrap gap-1 mt-1">
                              {analysis.complete.map((field) => (
                                <span
                                  key={field}
                                  className="bg-primary-800/50 px-2 py-1 rounded text-xs"
                                >
                                  {field.replace('_', ' ')}
                                </span>
                              ))}
                          ...
                          <div className="text-sm">
                            ...
                            <div className="flex flex-wrap gap-1 mt-1">
                              {analysis.priority.map((field) => (
                                <span
                                  key={field}
                                  className="bg-accent-800/50 px-2 py-1 rounded text-xs"
                                >
                                  {field.replace('_', ' ')}
                                </span>
                              ))}
