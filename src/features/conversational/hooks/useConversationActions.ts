...
export function useConversationActions(): UseConversationActionsReturn {
  ...
  const setValuationResult = useCallback(
  ...
  const setRegenerationWarning = useCallback(
    (show: boolean) => {
  ...
  const setPendingResult = useCallback(
    (result: ValuationResponse | null) => {
  ...
  const initializeSession = useCallback(
    (sessionId: string, pythonSessionId?: string | null, isRestored?: boolean) => {
      dispatch({ type: 'SET_SESSION_ID', payload: sessionId })
  ...
  const completeValuation = useCallback(
