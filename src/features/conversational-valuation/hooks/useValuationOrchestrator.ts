...
import { useConversationStateManager } from './useConversationStateManager'
...
export function useValuationOrchestrator({
  ...
  // Credit guard
  const { hasCredits, isBlocked, showOutOfCreditsModal, setShowOutOfCreditsModal } = useCreditGuard(
    {
      isAuthenticated,
      onOutOfCredits: () => {
        setStage('blocked')
